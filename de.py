...
rules = [
    ...
    SimpleRegexRule("Percent value not written as {\\,}\\%", r"(?<!\{\\,\}\\)%\s*\$", severity=Severity.standard),
    ...
    IgnoreByFilenameListWrapper(["de/4_low_priority/about.team.pot"],
            SimpleRegexRule("Occurrence of untranslated 'school'", r"(?<!Old-)(?<!High[- ])(?<!World[- ])\b[S]chool\b", severity=Severity.standard)),
    ...
    SimpleRegexRule("Missing translation of ten(s)", r"(?<!\d\{)\^?(\\large)?\\text\{\s*tens?\}\}", severity=Severity.dangerous),
